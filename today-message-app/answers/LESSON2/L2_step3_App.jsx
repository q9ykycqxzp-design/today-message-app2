...
function App() {
  ...
  const getTodayMessage = () => {
    // ステップ3: 気分（mood）と時間（time）の組み合わせをelse ifで追加する
    if (mood === 'challenge' && time === 'short') {
      setResult('短い時間で一歩目を踏み出して、課題や試験に手をつけよう！');
    } else if (mood === 'review' && time === 'short') {
      setResult('今日はサクッと復習をする1日♪学習したスライドを見直そう！');
    } else if (mood === 'relax' && time === 'short') {
      setResult('軽めの体操で体をほぐしながら、ゆったり動画を楽しもう！');
    } else if (mood === 'challenge' && time === 'long') {
      setResult('まとまった時間が取れた今こそ、新しい課題や試験に挑戦しよう！');
    } else if (mood === 'review' && time === 'long') {
      setResult('学んだことを丁寧に書き出して、頭を整理しよう！');
    } else if (mood === 'relax' && time === 'long') {
      setResult('気になったレッスンを視聴して、新しいことを発見しよう！');
    } else {
